...
* Loads all library metadata up front, caches it if it hasn't changed. Navigating music and searching in lists is instantaneous because you have all the data! Tested with library of about 9k songs. Normal data hit for start with no new content is about 11kb.
...
* You'll find middleware in the API project to cache-bust a few files to make sure changes update in the web app (in `Program.cs`). If you host the web app in a CDN or something, you'll need to set rules to not cache those.
...
* The rendering in Blazor for huge lists is extraordinary. My 9k song list is like butter.
